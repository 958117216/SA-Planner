/**
 * @file uu_ctrl_astar.h
 * @author Guo Yilin (958117216@qq.com)
 * @brief   Unclamped Uniform BSpline控制点 A*算法,与另一个cu_ctrl_astar.h 不能同时使用，原因 变量重名
 * @version 0.1
 * @date 2022-02-28
 * 
...
public:
  ...
  typedef std::shared_ptr<UUCtrlAstar> Ptr;
...
private:
    //此函数与 Clamped Uniform BSpline不同
    ...
    //指向性安全扩展
    vector<Eigen::Vector3d> DirectionSafeNodeExpansion(const vector<Eigen::Vector3d>& LocalCPs,const NodePtr& q_cur, const double step,const Eigen::Vector3d direction);
    
    Eigen::MatrixXd Directional_Search(Eigen::Vector3d pos_start,Eigen::Vector3d vel_start,Eigen::Vector3d acc_start,
            Eigen::Vector3d pos_goal,Eigen::Vector3d vel_goal,Eigen::Vector3d acc_goal);

vector<NodePtr> Stage_DirectionalExpand(vector<NodePtr>& q0q1q2_stage,const Eigen::Vector3d& goal_next,bool final_goal);
