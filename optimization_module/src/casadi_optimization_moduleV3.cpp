...
casadi_optimizationV3::casadi_optimizationV3(ros::NodeHandle& nh,const SWINMap::Ptr &map,const PlanningVisualization::Ptr& visualization)
{
    map_module_=map;
    visualization_=visualization;

    nh.param("casadi_optV3/max_vel",Vmax_,2.0);
    nh.param("casadi_optV3/max_acc",Amax_,2.0);
    nh.param("casadi_optV3/IterationNum",IterationNum_,1);
    nh.param("casadi_optV3/MaxCPNum",MaxCPNum_,20);
    nh.param("casadi_optV3/margin",margin_,0.2);
    nh.param("casadi_optV3/reduced_threshold",reduced_threshold_,2.0);
    nh.param("casadi_optV3/reduced_vel",reduced_vel_,0.4);
    nh.param("casadi_optV3/show_rviz",show_rviz_,true);
...
void casadi_optimizationV3::BuildNLP()
{
    for (int v=6; v<=MaxCPNum_ ;v++)
    {
        ...
        casadi::SX collision_constraints=SX::sym("collision_constraints",N_-4,IterationNum_);

        //p
        casadi::SX step=SX::sym("step",N_+1,1); //每个节点的step 目的是对应搜索的结果
        casadi::SX traj_goal=SX::sym("traj_goal",3,1);
        casadi::SX collision_Planes=SX::sym("collision_Planes",4,(N_-4)*IterationNum_);
        ...
        for (int i = 0; i < IterationNum_; ++i) //第i次迭代
        {
            for (int k = 3; k <= N_-2; ++k)
            {
                collision_constraints(k-3,i)=mtimes(q(Slice(),Slice(k,k+1)).T(),collision_Planes(Slice(0,3),Slice((k-3)+(N_-4)*i,(k-3)+(N_-4)*i+1)))+collision_Planes(3,(k-3)+(N_-4)*i);
            }
        }
...
bool casadi_optimizationV3::traj_optimization(const Eigen::MatrixXd &ctl_points,Eigen::MatrixXd &opt_ctl_points,
                                      const Eigen::RowVectorXd &step,const double &delta_t,const Eigen::MatrixXd &pos_goal)
{
    std::cout << "casadi_optimizationV3 优化"<<std::endl;
...
void casadi_optimizationV3::UpdateStepAndCollision_Constraints(Eigen::MatrixXd ctl_points)
{
...
bool casadi_optimizationV3::single_traj_optimization(Eigen::MatrixXd & IterationCtrlPoints)
{

    //========把那该死的数据填进去========================================================================
    casadi::DM lb_q,lb_vel_constraints,lb_acc_constraints,lb_step_constraints,lb_end_constraints,lb_collision_constraints;
    casadi::DM ub_q,ub_vel_constraints,ub_acc_constraints,ub_step_constraints,ub_end_constraints,ub_collision_constraints;
    ...
    //N_-4,IterationNum_
    lb_collision_constraints= casadi::DM::zeros(N_-4,IterationNum_);
    ub_collision_constraints= +inf*casadi::DM::ones(N_-4,IterationNum_);
    ...
    //p
    casadi::DM p_step,p_traj_goal,p_collision_Planes;
    ...
    //size: 4,(N_-4)*IterationNum_
    p_collision_Planes =casadi::DM::zeros(4,(N_-4)*IterationNum_);
    for (int i = 0; i <= IterNumNow_; ++i) {
        DM p_collision_Planes_valid = casadi::DM::ones(4,N_-4);
        std::copy(CollisionPlanes_[i].data(),CollisionPlanes_[i].data()+CollisionPlanes_[i].size(),p_collision_Planes_valid.ptr());
        p_collision_Planes.set(p_collision_Planes_valid,true,Slice(),Slice(i*(N_-4),i*(N_-4)+N_-4));
    }
