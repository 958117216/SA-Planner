/**
 * @file casadi_optimization_moduleV4.h
 * @author Guo Yilin (958117216@qq.com)
 * @brief  与V3不同，此版本会针对每段轨迹的最小凸包生成碰撞平面
 * @version 0.1
 * @date 2022-06-02
 * 
...
class casadi_optimizationV4
{

public:
    casadi_optimizationV4(ros::NodeHandle& nh,const SWINMap::Ptr &map,const PlanningVisualization::Ptr& visualization);
    ~casadi_optimizationV4(){}

    void BuildNLP();

     //主要接口, 进行轨迹优化
    bool traj_optimization(const Eigen::MatrixXd &pos_start,const Eigen::MatrixXd &vel_start,const Eigen::MatrixXd &acc_start,
...
private:
    ...
    std::vector<Eigen::MatrixXd> CollisionPlanes_;// vector 元素个数为: IterationNum_     每个元素为  4,num_of_segments_    4,(valid_N_-4)   
   ...
   conversion_BS2MV BS2MV;
    ...
    //转换矩阵  可提前定义的，设为类成员变量
    DM p_traj_goal_; //不会迭代更新的提前定义
    DM p_initial_pos_;
    DM p_initial_vel_;
    DM p_initial_acc_;
    DM p_deltaT_;

    DM p_jerk_bs_basis_matrix_; //固定矩阵
    DM p_pos_bs2mv_; //随着delta_t_会改变
